...
//        exit();
        ...
        $get_category_title_query = "select * from category where category_name='$category_name' and is_deleted='0'";
        ...
        $category_unique_slug = get_unique_slug1($db_mysqli, $category_name, 'category', 'category_name');

        $insert_category_query = "INSERT INTO category (category_name, category_unique_slug, category_images,meta_title, meta_keyword, meta_description, created_on, status, is_deleted) 
        VALUES ('$category_name','$category_unique_slug','$images','$meta_title','$search_keywords','$meta_description', '$created_on','$status', '0')";
        $result_insert_category_query = mysqli_query($db_mysqli, $insert_category_query);

        if ($result_insert_category_query)
        {
            $return["html_message"] = 'Category added successfully.';
