...
if ($admin == 1)
{
    if (isset($_POST))
    {
        ...
        $get_category_title_query = "select * from gift_subsubcate where giftsubsubcate_name='$giftsubsubcate_name' and giftsubcat_id='$giftsubcat_id' and is_deleted='0'";
        ...
        $giftsubsubcate_slug = '';
        $giftsubsubcate_slug = get_unique_slug1($db_mysqli, $giftsubsubcate_name, 'gift_subsubcate', 'giftsubsubcate_name');

        $insert_category_query = "INSERT INTO gift_subsubcate (gift_id,giftsubcat_id,giftsubsubcate_name, giftsubsubcate_slug, giftsubsubcate_images, meta_title, meta_keyword, meta_description, created_on, status, is_deleted) 
                                              VALUES ('$gift_id','$giftsubcat_id','$giftsubsubcate_name','$giftsubsubcate_slug','$images','$meta_title','$search_keywords','$meta_description', '$created_on','$status', '0');";
        $result_insert_category_query = mysqli_query($db_mysqli, $insert_category_query);

        if ($result_insert_category_query)
        {
            $return["html_message"] = 'Gift Sub Sub Category added successfully.';
