...
if ($admin == 1)
{
    $module_full_name = "Payment";
    $module_short_name = 'Payment';
    $module_name = "payment";
                                        ...
                                        <div class="row">
                                            <div class="col-md-4">
                                                <div class="form-group">
                                                    <label>Select Celebrity</label>
                                                    <select class="form-control" name="celebrity_id" id="celebrity_id" onchange="get_paymenthis(this.value);">
                                                        <option value="">Select Celebrity</option>
                                                        <?php
                                                            $get_celebrtity_query = "select * from user where user_type='2' and is_deleted='0'";
                                                            $result_get_celebrtity_query = mysqli_query($db_mysqli, $get_celebrtity_query);
                                                            while ($row_get_celebrtity_query = mysqli_fetch_assoc($result_get_celebrtity_query))
                                                            {
                                                                $check_celebrtity_data_array[] = $row_get_celebrtity_query;
                                                            } 

                                                            foreach($check_celebrtity_data_array as $check_celebrtity_data){
                                                        ?>
                                                        <option value="<?php echo $check_celebrtity_data['id']; ?>"><?php echo $check_celebrtity_data['user_name']; ?></option>
                                                        <?php } ?>
                                                    </select>
                                                </div>
                                            </div>
    ...
    <script type="text/javascript">
        ...
        function get_paymenthis(celebrity_id)
        {
            $.ajax(
                {
                    url: "<?php echo $base_url; ?>get_paymenthistory.php",
                    type: "POST",
                    ...
                    success: function (data)
                    {
                        ...
                        if (data.status == 'success')
                        {
                            $('#payment_div').html(data.html_message);
