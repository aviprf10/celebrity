...
if ($admin == 1)
{
    ...
    if (Secure1($db_mysqli, $requestData['search_subcategory_title']) != '')
    {
        ...
        $custom_filter .= " and c.giftsubcate_name LIKE '%" . $search_value . "%'";
    }
    ...
    if (Secure1($db_mysqli, $requestData['search_subsubcategory_title']) != '')
    {
        ...
        $custom_filter .= " and sc.giftsubsubcate_name LIKE '%" . $search_value . "%'";
    }
    ...
    $get_category_query = "select sc.*, c.giftsubcate_name from gift_subsubcate sc LEFT JOIN gift_subcat c on sc.giftsubcat_id = c.id where $custom_filter $order_by LIMIT $limit_start,$limit_end";
    $result_get_category_query = mysqli_query($db_mysqli, $get_category_query);

    $count_get_category_query = "select * from gift_subsubcate sc LEFT JOIN gift_subcat c on sc.giftsubcat_id = c.id where $custom_filter";
    ...
    if (count($all_category_data_array) > 0)
    {
        foreach ($all_category_data_array as $all_category_table_data)
        {
            ...
            $giftsubsubcate_name = $all_category_table_data['giftsubsubcate_name'];
            ...
            $nestedData[] = $giftsubsubcate_name;
            ...
            $nestedData[] = '
                <center>
                    <a href="' . $base_url . 'edit-gift-sub-sub-category/' . $row_id . '" title="Edit" class="tooltip_class" data-popup="tooltip"><i class="icon-pencil5 text-primary"></i></a>
                    &nbsp;&nbsp;
