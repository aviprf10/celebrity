...
//      else
//      {
//         $custom_filter .= " and (occasion_title LIKE '%".$query_string."%' or occasion_code LIKE '%".$query_string."%')";
//      }
//   }
    ...
    if(Secure1($db_mysqli, $requestData['search_category_title']) != '')
    {
        ...
        $custom_filter .= " and occasion_title LIKE '%" . $search_value . "%'";
    }
    ...
    $get_occasion_query = "select * from occasion where $custom_filter $order_by LIMIT $limit_start,$limit_end";
    $result_get_occasion_query = mysqli_query($db_mysqli, $get_occasion_query);

    $count_get_occasion_query = "select id from occasion where $custom_filter";
    $result_count_get_occasion_query = mysqli_query($db_mysqli, $count_get_occasion_query);
    $total_data = mysqli_num_rows($result_count_get_occasion_query);
    ...
    {
        foreach ($all_occasion_data_array as $all_occasion_table_data)
        {
            $row_id = $all_occasion_table_data['id'];
            $occasion_title = $all_occasion_table_data['occasion_title'];
            $status = $all_occasion_table_data['status'];
            ...
            $nestedData[] = '
                <center>
                    <a href="' . $base_url . 'edit-occasion/' . $row_id . '" title="Edit" class="tooltip_class" data-popup="tooltip"><i class="icon-pencil5 text-primary"></i></a>
                    &nbsp;&nbsp;
