...
if ($admin == 1)
{
    ...
    if (Secure1($db_mysqli, $requestData['search_title1']) != '')
    {
        ...
        $custom_filter .= " and offer_name LIKE '%" . $search_value . "%'";
    }
    ...
    $get_offer_banner_query = "select * from offer_banner where $custom_filter $order_by LIMIT $limit_start,$limit_end";
    $result_get_offer_banner_query = mysqli_query($db_mysqli, $get_offer_banner_query);

    $count_get_offer_banner_query = "select * from offer_banner where $custom_filter";
