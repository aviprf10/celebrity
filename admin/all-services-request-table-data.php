...
//   }
    ...
    $get_servicesrequest_query = "select * from services_request where $custom_filter $order_by LIMIT $limit_start,$limit_end";
    $result_get_servicesrequest_query = mysqli_query($db_mysqli, $get_servicesrequest_query);

    $count_get_servicesrequest_query = "select id from services_request where $custom_filter";
    $result_count_get_servicesrequest_query = mysqli_query($db_mysqli, $count_get_servicesrequest_query);
    $total_data = mysqli_num_rows($result_count_get_servicesrequest_query);

    $all_servicesrequest_data_array = array();
    while ($row_get_servicesrequest_query = mysqli_fetch_assoc($result_get_servicesrequest_query))
    {
        $all_servicesrequest_data_array[] = $row_get_servicesrequest_query;
    }


    $data = array();
    $count = 1;
    if (count($all_servicesrequest_data_array) > 0)
    {
        foreach ($all_servicesrequest_data_array as $all_servicesrequest_table_data)
        {
            $row_id = $all_servicesrequest_table_data['id'];
            $services_name = $all_servicesrequest_table_data['services_name'];
            $status = $all_servicesrequest_table_data['status'];
            ...
            // $nestedData[] = '
            //     <center>
            //         <a href="' . $base_url . 'edit-services-request/' . $row_id . '" title="Edit" class="tooltip_class" data-popup="tooltip"><i class="icon-pencil5 text-primary"></i></a>
            //         &nbsp;&nbsp;
            //         <a onclick="delete_row(' . $row_id . ')" class="tooltip_class" title="Remove" data-popup="tooltip"><i class="icon-trash text-danger "></i></a>
            //     </center>';    
