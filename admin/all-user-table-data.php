...
//    echo $requestData;
    ...
    $custom_query = "";
    $custom_filter = "u.is_deleted = '0' and u.user_type ='3'";
    ...
    $get_user_query = "SELECT u.id, CONCAT(u.first_name,' ',u.last_name) AS user_name, CONCAT(ul.first_name,' ',ul.last_name) AS refraluser_name, u.email, u.gender, u.mobile, u.user_type, u.status, u.status_by,u.password, u.instagram_link  FROM user u LEFT JOIN user ul on u.perant_id = ul.id where $custom_filter $order_by LIMIT $limit_start,$limit_end";

    $result_get_user_query = mysqli_query($db_mysqli, $get_user_query);

    $count_get_user_query = "SELECT u.id, CONCAT(u.first_name,' ',u.last_name) AS user_name, CONCAT(ul.first_name,' ',ul.last_name) AS refraluser_name, u.email, u.gender, u.mobile, u.user_type, u.status, u.status_by,u.password, u.instagram_link FROM user u LEFT JOIN user ul on u.perant_id = ul.id where $custom_filter";
    ...
    if (count($all_user_data_array) > 0)
    {
        foreach ($all_user_data_array as $all_user_data)
        {
            ...
            $password  = $all_user_data['password'];
            $gender    = $all_user_data['gender'];
            $mobile    = $all_user_data['mobile'];
            $status    = $all_user_data['status'];
            $user_type = $all_user_data['user_type'];
            $status_by = $all_user_data['status_by'];
            $instagram_link = $all_user_data['instagram_link'];
            ...
            if($user_type == '2')
            {
                $nestedData[] = 'Celebrity';
            }
            else 
            {
                $nestedData[] = 'Buyer User';
            }
            $nestedData[] = $email;
            $nestedData[] = $password;
            $nestedData[] = $gender;
            $nestedData[] = $mobile;
            $nestedData[] = $instagram_link;
            if ($status == '1')
            {
