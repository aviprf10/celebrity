...
if ($admin == 1 || ($sub_admin == 1 && $is_menu_write == 1))
{
	if(isset($_POST))
	{
		...
		if(count($all_menu_data_array) > 0)
		{
			...
			$check_menu_query = "select * from menu where branch_id='$branch_id' AND menu_name='$menu_name' AND id!='$edit_id' and is_deleted='0'";
			...
			else
			{
			 ...
			 $update_menu_query = "update menu set menu_name='$menu_name',branch_id='$branch_id',price='$price',status='$status', updated_on='$updated_on' where id='$edit_id'";
