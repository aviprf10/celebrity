...
if ($admin == 1)
{
    if ($_POST)
    {
        ...
        $get_sub_category_query = "SELECT * from sub_category WHERE category_id IN($category_id) and is_deleted=0";
        ...
        if(count($all_sub_category_data_array) > 0)
        {
            $html_message .= ' <div class="form-group">
                <label>Select Sub Category : </label>
                    <select class="select-search form-control" id="sub_category_id"  multiple="multiple" name="sub_category_id[]" data-placeholder="Select a Sub Category..." onchange="get_celebritydetails();">
                    <option></option>';
