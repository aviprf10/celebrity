...
{
    $module_full_name = 'Country';
    $module_short_name = 'Country';
    $module_name = 'country';
    ?>
    <!DOCTYPE html>
    <html lang="en">
...
<!--                                                <th>id</th>-->
                                                ...
                                                <th><?php echo $module_full_name; ?></th>
                                                <th>Status</th>
                                            ...
                                            <tr>
                                                <td></td>
                                                <td><input type="text" id="search_title" name="search_title"
...
//            $('.checker span').prop("indeterminate", true)
        ...
        {
//            var module_name = 'add-'+<?php //echo $module_name; ?>//;
            window.location.href = "<?php echo $base_url; ?>add-<?php echo $module_name; ?>";
        });

        Mousetrap.bind('/', function (e)
        {
            $.ajax({
                url: "<?php echo $base_url;?>modal-search-modules.php",
                type: "POST",
                dataType: 'json',
                encode: true,
                beforeSend: function ()
                ...
                success: function (data)
                {
                    ...
                    if (data.status == 'success')
                    {
                        $('#modal_response_search_modules_div').html(data.html_message);
                        $("#modal_search_modules").modal("show");
                        ...
                        $("#query").on("change paste keyup", function ()
                        {
                            ...
                            if (value != '')
                            {
                                $('.suggestion').addClass('open_class');
                            }
                            else
                            {
                                $('.suggestion').removeClass('open_class');
                            }
                            if (value != '')
                            {
                                $.ajax({
                                    ...
                                    success: function (data)
                                    {
                                        if (data.status === 'success')
                                        {
                                            var len = data.module_name.length;
                                            var content = '<div class="container-fluid">';
                                            for (var i = 0; i < len; i++)
                                            {
                                                content += '<div class="row">';
                                                content += '<div class="col-sm-10"><a href="' + data.module_link + '"><h4>' + data.module_name[i] + '</h4></div>';
                                                content += '</div>';
                                            }
                                            content += '</div>';
            ...
            if (value != '')
            {
                $.ajax({
                    ...
                    success: function (data)
                    {
                        if (data.status === 'success')
                        {
//                            $('#search_data').html(data.html_message);
//                            debugger;
                            ...
                            {
                                content += '<div class="row">';
                                content += '<div class="col-sm-10"><h4>' + data.text[i] + '</h4></div>';
                                content += '</div>';
                            }
                            content += '</div>';
                            //$('serch_data_outer').html('');
                            $('#search_data').html(content);
                        }
                        else
                        {
