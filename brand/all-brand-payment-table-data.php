...
if ($brand == 1)
{
    ...
    $custom_query = "";
    $custom_filter = "p.status='0' or p.status='1' or p.status='2' and p.brand_id='$loggedin_user_id'";
    ...
    if (Secure1($db_mysqli, $requestData['search_brand']) != '')
    {
        ...
        $custom_filter .= " and b.user_name = '" . $search_status . "'";
    }
    ...
    if (Secure1($db_mysqli, $requestData['search_debit_amt']) != '')
    {
        ...
        $custom_filter .= " and p.request_amount = '" . $search_status . "'";
    }
    ...
    $get_payment_query = "select p.*, u.user_name, b.user_name as brand_name from brand_payment_history p left join user u on p.admin_id=u.id left join brand_user b on p.brand_id=b.id where $custom_filter $order_by LIMIT $limit_start,$limit_end";
    $result_get_payment_query = mysqli_query($db_mysqli, $get_payment_query);

    $count_get_payment_query = "select p.*, u.user_name, b.user_name as brand_name from brand_payment_history p left join user u on p.admin_id=u.id left join brand_user b on p.brand_id=b.id where $custom_filter";
    ...
    if (count($all_payment_data_array) > 0)
    {
        foreach ($all_payment_data_array as $all_payment_table_data)
        {
            ...
            $brand_name = $all_payment_table_data['brand_name'];
            $created_amount = $all_payment_table_data['created_amount'];
            $request_amount = $all_payment_table_data['request_amount'];
            ...
            $nestedData[] = $request_amount;
            $nestedData[] = $amount;
            $nestedData[] = $created_on;
            if ($status == '0')
            {
                $nestedData[] = '<span class="btn btn-primery">Request added amount to admin</span>';
            }
            else
            {
                $nestedData[] = '<span class="btn btn-success">Paid By Brand</span>';
            }
