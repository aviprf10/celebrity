...
if($brand == 1)
{
	if(isset($_POST))
	{
		...
		if($new_password == $repeat_password)
		{
			...
			$user_data_query = "select * from brand_user where id='$loggedin_user_id'";
			...
			if(count($user_data_array)>0)
			{
				if($user_data_array[0]['password'] == $old_password)
				{
					if($old_password != $new_password)
					{
						...
						$user_data_query = "update brand_user set password='$password', updated_on='$updated_on' where id='$loggedin_user_id'";
