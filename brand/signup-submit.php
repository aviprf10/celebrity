...
if (isset($_POST['email']))
{
    ...
    $get_branduser_query = "select * from brand_user where email='$email'";
    ...
    $get_mobile_query = "select * from brand_user where  mobile='$mobile'";
    ...
    {
        $return["html_message"] = 'Mobile all ready register please try an other!.';
        $return["status"] = "error";
        echo json_encode($return);
        exit();
    }
    else 
    {
        if($password == $confirm_password)
        {
            ...
            $insert_user_query = "INSERT INTO brand_user (name,user_name, user_unique_slug,email,password,mobile,registration_date,user_type,unique_key,ip_address,created_on,status,facebook_link,twitter_link,instagram_link,whatsapp_link,linkedin_link,website_url,description) VALUES ('$name', '$user_name', '$user_unique_slug','$email','$pass','$mobile', '$date','1','$unique_key','$ip_address','$date','2','$facebook_link','$twitter_link','$instagram_link','$whatsapp_link','$linkedin_link','$website','$description');";
            $result_insert_user_query = mysqli_query($db_mysqli, $insert_user_query);
            $login_id = mysqli_insert_id($db_mysqli);
            if ($result_insert_user_query) {
                ...
                $get_user_query = "select * from brand_user where id='$login_id' and is_deleted=0 and user_type='1'";
                ...
                $return["html_message"] = 'Brand registration ruccessfully. your acount under process';
