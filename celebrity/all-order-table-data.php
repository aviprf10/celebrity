...
if ($celebrity == 1)
{
    ...
    if($current_order_status == 'new_order')
    {
        ...
        $custom_query = "";
        $custom_filter = "uo.is_deleted= '0' and (uo.order_status='0' or uo.order_status='2' or uo.order_status='6') and uo.celebrity_id='$loggedin_user_id'";
        ...
        if (Secure1($db_mysqli, $requestData['search_product_name']) != '')
        {
            ...
            $custom_filter .= " and c.user_name LIKE '%" . $search_value . "%'";
        }
        ...
        $get_product_query = "select uo.id, o.occasion_title,uo.date_of_delevery,uo.order_id, uo.order_date, uo.order_status, c.user_name as celebrty_name, u.user_name, u.mobile from user_order uo  LEFT JOIN occasion o on uo.occasion_id = o.id LEFT JOIN user u on uo.user_id = u.id LEFT JOIN user c on uo.celebrity_id = c.id where $custom_filter group by order_id $order_by LIMIT $limit_start,$limit_end";
        $result_get_product_query = mysqli_query($db_mysqli, $get_product_query);

        $count_get_product_query = "select uo.id, o.occasion_title,uo.date_of_delevery,uo.order_id, uo.order_date, uo.order_status, c.user_name as celebrty_name, u.user_name, u.mobile from user_order uo  LEFT JOIN occasion o on uo.occasion_id = o.id LEFT JOIN user u on uo.user_id = u.id LEFT JOIN user c on uo.celebrity_id = c.id where $custom_filter group by order_id";
    ...
    else if($current_order_status == 'returned_order')
    {
        ...
        $custom_query = "";
        $custom_filter = "uo.is_deleted= '0' and uo.order_status='7' and uo.celebrity_id='$loggedin_user_id'";
    ...
    else if($current_order_status == 'rejected_order')
    {
        ...
        $custom_query = "";
        $custom_filter = "uo.is_deleted= '0' and (uo.order_status='8' or uo.order_status='9') and uo.celebrity_id='$loggedin_user_id'";
        ...
        $get_product_query = "select uo.id, o.occasion_title,uo.cancel_order_date as date_of_delevery,uo.order_id, uo.order_date, uo.order_status, c.user_name as celebrty_name, u.user_name, u.mobile from user_order uo  LEFT JOIN occasion o on uo.occasion_id = o.id LEFT JOIN user u on uo.user_id = u.id LEFT JOIN user c on uo.celebrity_id = c.id where $custom_filter group by order_id $order_by LIMIT $limit_start,$limit_end";
        $result_get_product_query = mysqli_query($db_mysqli, $get_product_query);

        $count_get_product_query = "select uo.id, o.occasion_title,uo.cancel_order_date as date_of_delevery,uo.order_id, uo.order_date, uo.order_status, c.user_name as celebrty_name, u.user_name, u.mobile from user_order uo  LEFT JOIN occasion o on uo.occasion_id = o.id LEFT JOIN user u on uo.user_id = u.id LEFT JOIN user c on uo.celebrity_id = c.id where $custom_filter group by order_id";
        $result_count_get_product_query = mysqli_query($db_mysqli, $count_get_product_query);
        $total_data = mysqli_num_rows($result_count_get_product_query);
    ...
    if (!empty($all_product_data_array))
    {
        foreach ($all_product_data_array as $all_product_table_data)
        {
            ...
            $celebrty_name = $all_product_table_data['celebrty_name'];
            $occasion_title = $all_product_table_data['occasion_title']. 'Messages';
            $date_of_delevery= date('d-m-Y', strtotime($all_product_table_data['date_of_delevery']));
