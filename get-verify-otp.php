...
header('Content-type: application/json');
	...
	if($_POST)
	{
		...
		$all_user_data_array = array();
      	$get_user_query = "select * from  user where  email='$email' and is_deleted = '0' and user_type='3' and status='1'";
     	$result_get_user_query = mysqli_query($db_mysqli, $get_user_query);
      	...
      	if(isset($all_user_data_array) && count($all_user_data_array)==0)
      	{
			...
			if($email==$_SESSION['email_'.$company_name_session])
			{	
				...
				{	
					if($input_otp==$_SESSION['otp_'.$company_name_session])
					{
						...
						$return["html_message"] = 'Email Number Verified Successfully!';
						$return["status"] = "success";
						$return["mobile_verification_status"] = " Verified";
						echo json_encode($return);
						exit();
					}
					else
