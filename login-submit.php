...
{ 
    ...
    $get_user_query = "select * from user where (email='$login_id' or  mobile='$login_id') and is_deleted=0 and user_type='3'";
    ...
    if ($valid_user == 1 && ( $user_data['registered_from'] == ''))
    {
        ...
        else if ($user_data['status'] == 1)/***** Account is Active. *****/
        {
            ...
            $module_user = "update user set last_login='$last_login', mobile_access_token='$mobile_access_token',mobile_token_exp_date='$mobile_token_exp_date' where id='$user_id'";
            ...
            if(isset($_SESSION['cart_'.$company_name_session]) && (count($_SESSION['cart_'.$company_name_session])>0)) 
            {
                ...
                foreach($_SESSION['cart_'.$company_name_session] as $key => $value)
                {
                    ...
                    $from_name = $_SESSION['cart_'.$company_name_session][$key]['from_name'];
                    $type_id = $_SESSION['cart_'.$company_name_session][$key]['type_id'];
                    $instagramid = $_SESSION['cart_'.$company_name_session][$key]['instagramid'];
                    ...
                    {   
                        ...
                        $update_user_cart_query = "update user_cart set name='$name',from_name='$from_name', type='$type_id',occasion_id='$occasion_id',date_of_delevery='$date_of_delevery', price='$price', user_message='$user_message',celebrity_id='$celebrity_id', user_message='$user_message', message_id='$message_id',request_for='$request_for',order_date='$order_date', order_date_time='$order_date_time', celebrity_price_id='$celebrity_price_id', services_id='$services_id', discount_type='$discount_type', discount='$discount', instagramid='$instagramid' where user_id='$loggedin_user_id' and celebrity_id='$celebrity_id'";
            ...
            $total_user_cart_data_array = array();
            $get_user_cart_query = "select * from user_cart where user_id='$user_id'";
            $result_user_cart_data = mysqli_query($db_mysqli,$get_user_cart_query);
            while ($row_user_cart_data = mysqli_fetch_assoc($result_user_cart_data))
            {
                $total_user_cart_data_array[] = $row_user_cart_data;
            } 
            $total_user_cart_data=count($total_user_cart_data_array);
            $_SESSION['total_user_cart_data_'.$company_name_session] = $total_user_cart_data; 
            ...
            else if($user_type == 3 && ($total_user_cart_data == 0 || $total_user_cart_data == '') &&   $login_id_session != '') /***** User is member *****/
            {
                
                $return["page"] = "account-dashboard";
                $return["total_user_cart_data"] = $total_user_cart_data;
                $return["html_message"] = '';
                $return["status"] = "success";
                echo json_encode($return);
                exit(); 
            ...
            else if($user_type == 3 && $total_user_cart_data > 0 || $login_id_session == 'checkout.php') /***** User is member *****/
            {
